// 안정화 패치: 첫 제목 반짝임, 선택 도장 1회 애니메이션, 마지막 결론 도장
(function(){
  const VERSION = '2026-05-27-animation-bugfix-v14-single-impact';
  if(window.__ANIMATION_BUGFIX__ === VERSION) return;
  window.__ANIMATION_BUGFIX__ = VERSION;

  let refreshTimer = null;
  let observerStarted = false;
  const impactedChoiceKeys = window.__STAMP_IMPACTED_CHOICE_KEYS__ || new Set();
  window.__STAMP_IMPACTED_CHOICE_KEYS__ = impactedChoiceKeys;
  ...
  function addStyles(){
    ...
    style.textContent = `
      ...
      .feedback .verdict-stamp.stamp-impact{animation:choiceStampImpact .48s cubic-bezier(.16,1.08,.28,1) both!important}
      @keyframes choiceStampImpact{
        0%{opacity:0;transform:translateY(-24px) rotate(-14deg) scale(2.15);filter:blur(2px);box-shadow:0 0 0 rgba(141,47,39,0)}
        58%{opacity:1;transform:translateY(0) rotate(-5deg) scale(1);filter:none;box-shadow:inset 0 0 0 2px rgba(141,47,39,.34),0 0 0 9px rgba(141,47,39,.16),0 12px 24px rgba(141,47,39,.18)}
  ...
  function ensureIntroSparkle(slide, force){
    ...
    const h1 = slide.querySelector('h1');
    if(!h1) return;
    if(h1.dataset.sparkleVersion !== 'v14'){
  ...
  function wrap(name){
    ...
    if(typeof original !== 'function' || original.__animationBugfixWrappedV14) return;
    function wrapped(){
      ...
      schedule(90, forceIntro);
      setTimeout(() => refresh(forceIntro), 280);
      return result;
    }
    wrapped.__animationBugfixWrappedV14 = true;
    window[name] = wrapped;
  }
  ...
  function startObserver(){
    ...
    new MutationObserver(() => schedule(50, false)).observe(app, {childList:true, subtree:false});
  }

  document.addEventListener('click', event => {
    if(event.target.closest('.choice,[data-choice],button,a')){
      setTimeout(() => refresh(false), 160);
    }
  }, true);
