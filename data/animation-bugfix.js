...
(function(){
  const VERSION = '2026-05-27-animation-bugfix-v10-focused';
  ...
  function addStyles(){
    ...
    style.textContent = `
      ...
      .feedback .verdict-stamp.stamp-impact{
        animation:choiceStampImpact .66s cubic-bezier(.15,1.55,.22,1) both!important;
      }
      @keyframes choiceStampImpact{
        0%{opacity:0;transform:translateY(-18px) rotate(-16deg) scale(2.25);filter:blur(2px);box-shadow:0 0 0 rgba(141,47,39,0)}
        38%{opacity:1;transform:translateY(2px) rotate(-5deg) scale(.76);filter:none;box-shadow:inset 0 0 0 2px rgba(141,47,39,.34),0 0 0 8px rgba(141,47,39,.16),0 10px 22px rgba(141,47,39,.18)}
        55%{transform:translateY(0) rotate(-5deg) scale(1.12);box-shadow:inset 0 0 0 1px rgba(141,47,39,.3),0 0 0 2px rgba(141,47,39,.08),0 7px 16px rgba(141,47,39,.14)}
        74%{transform:translateY(0) rotate(-5deg) scale(.96)}
      ...
      .final-completion-stamp{
        ...
        bottom:clamp(4.5rem,13vh,8rem)!important;
      ...
      .final-completion-stamp.final-stamp-impact{
        animation:finalStampImpact .78s cubic-bezier(.15,1.45,.22,1) both!important;
      }
  ...
  function ensureIntroSparkle(slide, force){
    ...
    if(h1.dataset.sparkleVersion !== 'v10'){
  ...
  function ensureChoiceStamp(slide, force){
    if(!slide) return;
    slide.querySelectorAll('.feedback.visible, .feedback.show, .feedback.active').forEach(feedback => {
      ...
      const textKey = feedback.textContent.trim().slice(0,80);
      if(!force && stamp.dataset.lastImpactKey === textKey) return;
      stamp.dataset.lastImpactKey = textKey;
  ...
  function wrap(name){
    ...
    if(typeof original !== 'function' || original.__animationBugfixWrappedV10) return;
    function wrapped(){
      ...
      schedule(70, forceIntro, true);
      setTimeout(() => refresh(forceIntro, true), 260);
      return result;
    }
    wrapped.__animationBugfixWrappedV10 = true;
    window[name] = wrapped;
  }

  function hook(){
    ['render','goTo','linkedGo','nextSlide','prevSlide','resetPresentation','startTimeTravel'].forEach(wrap);
  }
  ...
  function startObserver(){
    ...
    new MutationObserver(() => schedule(30, false, false)).observe(app, {childList:true, subtree:false});
  }

  document.addEventListener('click', event => {
    if(event.target.closest('.choice,[data-choice],button,a')){
      setTimeout(() => refresh(false, true), 80);
      setTimeout(() => refresh(false, true), 220);
      setTimeout(() => refresh(false, true), 420);
    }
  }, true);
