// 애니메이션 안정화 패치
// 담당 범위: ① 첫 제목 글자별 반짝임 ② 선택 피드백 도장 1회 애니메이션 ③ 마지막 결론 도장
(function(){
  const VERSION = '2026-05-27-animation-bugfix-v16-clean-stamp';
  ...
  const stampedChoiceKeys = new Set();
  ...
  function addStyles(){
    ...
    style.textContent = `
      ...
      .feedback .verdict-stamp.stamp-impact{animation:choiceStampImpact .54s cubic-bezier(.16,1.14,.28,1) both!important}
      @keyframes choiceStampImpact{
        0%{opacity:0;transform:translateY(-28px) rotate(-15deg) scale(2.15);filter:blur(2px);box-shadow:0 0 0 rgba(141,47,39,0)}
        54%{opacity:1;transform:translateY(0) rotate(-5deg) scale(.90);filter:none;box-shadow:inset 0 0 0 2px rgba(141,47,39,.34),0 0 0 10px rgba(141,47,39,.16),0 12px 24px rgba(141,47,39,.18)}
        76%{opacity:1;transform:translateY(0) rotate(-5deg) scale(1.04);filter:none}
      ...
      .final-completion-stamp.final-stamp-impact{animation:finalStampImpact .62s cubic-bezier(.16,1.12,.28,1) both!important}
      @keyframes finalStampImpact{
        0%{opacity:0;transform:translate3d(0,-160px,0) rotate(-17deg) scale(1.8);filter:blur(2px)}
        58%{opacity:1;transform:translate3d(0,0,0) rotate(-7deg) scale(.93);filter:none}
        78%{opacity:1;transform:translate3d(0,0,0) rotate(-7deg) scale(1.04);filter:none}
  ...
  function isConclusionSlide(slide){
    return !!(slide && getSlideCount() > 0 && getCurrent() === getSlideCount() - 1);
  }
  ...
  function ensureIntroSparkle(slide, force){
    ...
    if(h1.dataset.sparkleVersion !== 'v16'){
  ...
  function feedbackKey(feedback, stamp){
    return normalizeText(Array.from(feedback.childNodes)
  ...
  function ensureChoiceStamp(slide){
    ...
    visibleFeedbacks(slide).forEach(feedback => {
      ...
      const key = idx + '|' + feedbackKey(feedback, stamp);
      stamp.style.animation = '';
      stamp.style.opacity = '1';
      stamp.style.transform = 'rotate(-5deg) scale(1)';
      stamp.style.filter = 'none';

      if(!pendingChoice || pendingChoice.slide !== idx || pendingChoice.used || pendingChoice.expires < Date.now()) return;
      if(!key || stampedChoiceKeys.has(key)) return;

      stampedChoiceKeys.add(key);
      pendingChoice.used = true;
  ...
  function wrap(name){
    ...
    if(typeof original !== 'function' || original.__animationBugfixWrappedV16) return;
    function wrapped(){
      if(name === 'resetPresentation') stampedChoiceKeys.clear();
    ...
    wrapped.__animationBugfixWrappedV16 = true;
    window[name] = wrapped;
  }
  ...
  document.addEventListener('click', event => {
    if(event.target.closest('.choice,[data-choice]')){
      pendingChoice = {slide:getCurrent(), used:false, expires:Date.now() + 1200};
      setTimeout(() => refresh(false), 140);
      setTimeout(() => refresh(false), 360);
    }
  }, true);
