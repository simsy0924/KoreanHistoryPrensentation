// 가벼운 먹물 전환: 일반 슬라이드 이동에만 재생, 타임머신/1페이지 이동과 충돌하지 않음
(function(){
  const VERSION = '2026-05-27-ink-transition-lite-v4-skip-first';
  ...
  function addStyles(){
    ...
    style.textContent = `
      ...
      #inkLiteSweep .ink-lite-brush{position:absolute;top:-22vh;bottom:-22vh;left:-44vw;width:48vw;transform:translateX(-76vw) rotate(-8deg) skewX(-7deg);opacity:0;border-radius:48% 52% 58% 42% / 38% 62% 45% 55%;clip-path:polygon(10% 0,92% 6%,82% 20%,100% 38%,78% 56%,92% 74%,68% 100%,0 92%,14% 72%,4% 52%,18% 30%,0 11%);background:radial-gradient(circle at 18% 18%,rgba(245,234,210,.20) 0 1.15vmin,transparent 1.75vmin),radial-gradient(circle at 72% 28%,rgba(245,234,210,.14) 0 .95vmin,transparent 1.55vmin),radial-gradient(circle at 50% 80%,rgba(245,234,210,.12) 0 1.15vmin,transparent 1.8vmin),linear-gradient(90deg,rgba(7,6,4,0),rgba(7,6,4,.62) 13%,rgba(3,3,2,.94) 43%,rgba(4,3,2,.9) 58%,rgba(20,14,9,.72) 77%,rgba(201,154,58,.18) 89%,rgba(7,6,4,0));filter:drop-shadow(0 0 13px rgba(0,0,0,.62)) drop-shadow(0 0 8px rgba(201,154,58,.12));will-change:transform,opacity}
      #inkLiteSweep .ink-lite-edge{position:absolute;top:-16vh;bottom:-16vh;left:-22vw;width:9vw;opacity:0;border-radius:50%;transform:translateX(-48vw) rotate(-8deg);background:linear-gradient(90deg,transparent,rgba(245,234,210,.32),rgba(201,154,58,.27),transparent);filter:blur(5px);will-change:transform,opacity}
  ...
  function isTimeMachineActive(){return !!document.querySelector('.timewarp.active,#timewarp.active,.return-present.active,#returnPresentEffect.active')}
  ...
  function isReturnToPresentTarget(target){const count=getSlideCount();const idx=getCurrent();return count>1&&idx===count-2&&target===count-1}
  function isNextReturnToPresent(){const count=getSlideCount();const idx=getCurrent();return count>1&&idx===count-2}
  function isFirstPageTarget(target){return target===0}
  function isPrevToFirst(){return getCurrent()===1}

  function isTimeMachineOrFirstTrigger(args, fnName){
    if(fnName === 'startTimeTravel') return true;
    if(fnName === 'nextSlide' && isNextReturnToPresent()) return true;
    if(fnName === 'prevSlide' && isPrevToFirst()) return true;
    const first = args && args.length ? args[0] : null;
    const target = typeof first === 'number' ? first : parseInt(first, 10);
    if(Number.isFinite(target) && isFirstPageTarget(target)) return true;
    return Number.isFinite(target) && isReturnToPresentTarget(target);
  }

  function parseTargetFromOnclick(code){
    const goToMatch = code.match(/goTo\(([^)]+)\)|linkedGo\(([^),]+)/);
    if(!goToMatch) return NaN;
    const expr = (goToMatch[1] || goToMatch[2] || '').trim();
    try{
      if(expr === 'slides.length-1' || expr === 'slides.length - 1') return getSlideCount() - 1;
  ...
  function shouldSkipClick(button){
    ...
    if(/startTimeTravel|발표 시작|현재로 돌아|timewarp|return|초기화|처음|1페이지|첫 페이지/i.test(code + ' ' + text)) return true;
    if(/nextSlide/.test(code) && isNextReturnToPresent()) return true;
    if(/다음/.test(text) && isNextReturnToPresent()) return true;
    if(/prevSlide/.test(code) && isPrevToFirst()) return true;
    if(/이전/.test(text) && isPrevToFirst()) return true;
    const target = parseTargetFromOnclick(code);
    if(Number.isFinite(target) && (isFirstPageTarget(target) || isReturnToPresentTarget(target))) return true;
    return false;
  }
  ...
  function playInk(){
    ...
    setTimeout(()=>{overlay.classList.remove('active');playing=false},1000);
  }

  function wrap(name){
    const original = window[name];
    if(typeof original !== 'function' || original.__inkTransitionLiteWrappedV4) return;
    function wrapped(){
      const args = arguments;
      if(!isTimeMachineOrFirstTrigger(args, name)) playInk();
      return original.apply(this, args);
    }
    wrapped.__inkTransitionLiteWrappedV4 = true;
    window[name] = wrapped;
  }
