// 발표 보조 레이어
// 목표: 예전 문어발식 보조 파일들의 핵심 역할을 이 파일 하나에 통합한다.
// 포함 기능: 윤치호 카드 보정, 먹 웨이브 전환, 인트로 글자 등장, 선택 도장, 결론 클라이맥스, 황금 입자, 복귀 연출.
(function(){
  'use strict';

  const VERSION = '2026-06-10-unified-cinematic-enhancements-v1';
  ...
  function addStyles(){
    ...
    style.textContent = `
      ...
      @keyframes glitchSlice{0%,100%{clip-path:inset(0);transform:translate(0,0);filter:none}12%{clip-path:inset(18% 0 62% 0);transform:translate(-5px,0);filter:hue-rotate(20deg)}24%{clip-path:inset(72% 0 10% 0);transform:translate(4px,0)}36%{clip-path:inset(40% 0 42% 0);transform:translate(-2px,1px);filter:hue-rotate(-12deg)}50%{clip-path:inset(0 0 82% 0);transform:translate(3px,-1px)}70%{clip-path:inset(0)}}
      ...
      /* ===== 먹 웨이브 전환: 예전 cinematic-effects 계열을 현재 goTo 구조에 맞게 통합 ===== */
      .ink-sweep{position:fixed;inset:0;z-index:9998;pointer-events:none;opacity:0;will-change:opacity;background:rgba(7,6,4,0)}
      .ink-sweep.active{opacity:1;background:rgba(7,6,4,.12)}
      .ink-sweep svg{width:100%;height:100%;display:block}.ink-sweep path{will-change:d}
      .ink-sweep .ink-splatter{position:absolute;inset:0;background-image:radial-gradient(circle at 14% 28%,#0a0907 0 .65vmin,transparent .9vmin),radial-gradient(circle at 82% 65%,#0a0907 0 .85vmin,transparent 1.1vmin),radial-gradient(circle at 48% 18%,#0a0907 0 .55vmin,transparent .8vmin),radial-gradient(circle at 22% 78%,#0a0907 0 .75vmin,transparent 1vmin),radial-gradient(circle at 70% 38%,#0a0907 0 .65vmin,transparent .9vmin),radial-gradient(circle at 35% 52%,rgba(0,0,0,.72) 0 3.8vmin,transparent 4.4vmin);opacity:0;transition:opacity .28s ease;filter:blur(.2px)}
      .ink-sweep.splatting .ink-splatter{opacity:.9}
      ...
      .final-verdict-stamp{position:absolute;right:8%;bottom:14%;width:clamp(5.2rem,11vmin,9rem);height:clamp(5.2rem,11vmin,9rem);border:.42rem solid #8d2f27;border-radius:14%;display:grid;place-items:center;color:#8d2f27;font-family:"Noto Serif KR",serif;font-weight:900;font-size:clamp(2.4rem,6vmin,4.4rem);opacity:0;transform:translate(0,-200px) rotate(-25deg) scale(2.4);filter:blur(6px);background:rgba(141,47,39,.04);box-shadow:inset 0 0 0 .15rem rgba(141,47,39,.4);pointer-events:none;z-index:5}.final-verdict-stamp.drop{animation:stampDrop .58s cubic-bezier(.5,1.7,.4,1) forwards,stampSettle 1.8s ease-out .58s forwards}@keyframes stampDrop{0%{opacity:0;transform:translate(0,-200px) rotate(-25deg) scale(2.4);filter:blur(6px)}60%{opacity:1;transform:translate(0,10px) rotate(-9deg) scale(.9);filter:blur(0)}100%{opacity:1;transform:translate(0,0) rotate(-7deg) scale(1);filter:blur(0)}}@keyframes stampSettle{0%{box-shadow:inset 0 0 0 .15rem rgba(141,47,39,.4)}100%{box-shadow:inset 0 0 0 .15rem rgba(141,47,39,.6),0 0 32px rgba(141,47,39,.4)}}
  ...
  function createInkOverlay(){
    ...
    wrap.innerHTML = `<svg viewBox="0 0 100 100" preserveAspectRatio="none"><defs><filter id="inkTurb" x="-20%" y="-20%" width="140%" height="140%"><feTurbulence type="fractalNoise" baseFrequency="0.018 0.05" numOctaves="2" seed="3"/><feDisplacementMap in="SourceGraphic" scale="14"/></filter></defs><path id="inkPath" fill="#0a0907" ${isLowEnd ? '' : 'filter="url(#inkTurb)"'} d="M-20,0 L-20,100 L-20,100 L-20,0 Z"/></svg><div class="ink-splatter"></div>`;
  ...
  function playInkSweep(onMidpoint, duration){
    return new Promise(resolve => {
      ...
      const path = $('#inkPath', wrap);
      const DURATION = duration || (isLowEnd ? 620 : 930);
      ...
      function frame(now){
        ...
        path.setAttribute('d', `M${rightEdge - 32},0 L${leftEdge},0 L${leftEdge + 8},100 L${rightEdge - 26},100 Z`);
        if(t >= 0.5 && !midCalled){
          midCalled = true;
  ...
  function playReturnAndGo(target){
    ...
    setTimeout(() => { goDirect(target); afterSlideChange(); }, 980);
    setTimeout(() => { overlay.classList.remove('active'); isReturning = false; afterSlideChange(); }, 3200);
  }
  ...
  function afterSlideChange(previousStage){
    setTimeout(() => {
      const idx = currentIndex();
      if(idx === 0){ resetIntroAnimation(); }
  ...
  function installNavigationHooks(){
    ...
    window.goTo = function(target, scroll=true){
      ...
      const duration = indexDelta >= 2 ? 1220 : 930;
      playInkSweep(() => {
        goDirect(safeTarget, scroll);
        afterSlideChange(previousStage);
      }, duration).then(() => { isNavigating = false; });
    };

    window.nextSlide = function(){
      return window.goTo(Math.min(currentIndex() + 1, lastIndex()));
    };
    window.prevSlide = function(){
      return window.goTo(Math.max(currentIndex() - 1, 0));
    };
    ...
    if(typeof rawStartTimeTravel === 'function'){
      window.startTimeTravel = function(){
        ...
        setTimeout(afterSlideChange, 2860);
        setTimeout(afterSlideChange, 3460);
        return result;
      };
    }
    ...
    document.addEventListener('click', event => {
      ...
      if(event.target.closest('[data-next],#nextBtn')){
        event.preventDefault();
        event.stopImmediatePropagation();
        window.nextSlide();
      }else if(event.target.closest('#prevBtn')){
        event.preventDefault();
        event.stopImmediatePropagation();
        window.prevSlide();
      }
